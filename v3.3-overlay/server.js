...
async function readJsonBody(req, maxBytes = 32768) {
  return await new Promise((resolve, reject) => {
    ...
    req.on("end", () => {
      ...
      catch { const error = new Error("Invalid JSON request."); error.status = 400; reject(error); }
    });
    req.on("error", reject);
  });
}
function qBounds(p) {
  return { lamin: p.get("lamin"), lomin: p.get("lomin"), lamax: p.get("lamax"), lomax: p.get("lomax") };
}
...
async function api(req, res, url) {
  ...
  try {
    ...
    if (precipitationTile) {
      ...
      res.setHeader("X-SkyTrace-Tile-Cache", tile.cache || "MISS");
    ...
    if (url.pathname === "/api/flights") {
      ...
      return json(res, 200, await getFreeFlightSnapshot({
    ...
    if (url.pathname === "/api/airports") {
      if (p.get("code")) {
        ...
        return airport ? json(res, 200, { ok: true, source: "OurAirports", airport }) : json(res, 404, { ok: false, error: "Airport not found" });
      }
...
function staticFile(res, pathname) {
  if (pathname === "/config.json" || pathname === "/config.example.json" || pathname.startsWith("/lib/config")) {
    res.statusCode = 404;
...
if (isDirectRun) {
  startSkyTraceServer().catch((error) => { console.error(error); process.exitCode = 1; });
}

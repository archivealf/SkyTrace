...
async function getCsv(url) {
  ...
  const response = await fetch(url, {
    headers: { "User-Agent": "SkyTrace-data-builder/3.1" }
  });
...
async function main() {
  ...
  const [airportRows, runwayRows, frequencyRows, navaidRows] = await Promise.all([
  ...
  const airports = airportRows.map((r) => ({
    ...
    type: r.type || "",
    name: r.name || "",
    lat: num(r.latitude_deg, 0),
    lon: num(r.longitude_deg, 0),
    elev: num(r.elevation_ft),
  ...
  for (const r of runwayRows) {
    ...
    (runways[ident] ||= []).push({
      ...
      le: r.le_ident || "",
      he: r.he_ident || "",
      leh: num(r.le_heading_degT),
      heh: num(r.he_heading_degT)
    });
  }
  ...
  for (const r of frequencyRows) {
    const ident = r.airport_ident || "";
    if (!ident) continue;
    (frequencies[ident] ||= []).push({
      type: r.type || "",
      desc: r.description || "",
      mhz: num(r.frequency_mhz)
    });
  }
  ...
  const navaids = navaidRows.map((r) => ({
    ...
    name: r.name || "",
    type: r.type || "",
    khz: num(r.frequency_khz),
    lat: num(r.latitude_deg, 0),
    lon: num(r.longitude_deg, 0),
    country: r.iso_country || "",
    airport: r.associated_airport || ""
  }));

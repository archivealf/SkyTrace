...
# OpenSky is deliberately not part of the monetized runtime. The commercial
# build uses providers with terms that allow the use described in ATTRIBUTION.md.
...
# committing secrets. Provider selection is fixed by the overlay modules rather
# than by weakening or bypassing any provider's commercial terms.
